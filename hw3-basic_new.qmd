...
translated all the characters values into factors (nominal). This is often needed for building models.
...
displaying the summary of artists
...
```
This decision tree predicts the book ("Gardner" or "Janson") based on the features in the dataset. The tree splits the data into subsets based on specific conditions related to **edition_number** and **year**.
...
```
When **cp = 0**, the tree is allowed to grow until it perfectly fits the training data, which can result in a very complex, deep, and overfit tree.
...
**gain.ratio** function calculates the gain ratio for each predictor variable.
